...
def parse_diann(
    ...
    quantification_file = quantification_file[
        quantification_file[sample_q_value_column] <= diann_qvalue
    ]
    ...
    wide_results[[col for col in wide_results.columns if col not in scoring_columns]].replace(0, np.nan, inplace=True)
