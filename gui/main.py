...
with ui.splitter(value=20).classes('w-full') as splitter:
    with splitter.before:
        with ui.tabs().props('vertical') as tabs:
            ...
            normalization = ui.tab('Normalize').bind_visibility_from(normalize, 'value')
            batch_correction = ui.tab('Correct').bind_visibility_from(correct, 'value')
            protein_quantification = ui.tab('Quantify').bind_visibility_from(quantify, 'value')
            imputation = ui.tab('Impute').bind_visibility_from(impute, 'value')
            differential_abundance = ui.tab('Compare').bind_visibility_from(compare, 'value')
            xml = ui.tab('Explain').bind_visibility_from(explain, 'value').bind_visibility_from(explain, 'value')
            feature_selection = ui.tab('Select').bind_visibility_from(select, 'value')
            run = ui.tab('Run')
    ...
    with splitter.after:
        with ui.tab_panels(tabs, value=home).props('vertical').classes('w-full h-full'):
            ...
            with ui.tab_panel(normalization).bind_visibility_from(normalize, 'value'):
